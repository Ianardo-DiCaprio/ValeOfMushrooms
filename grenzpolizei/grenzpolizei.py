            ...
            for key in all_settings:
                ...
                if key == 'events':
                    ...
                    for event in all_settings['events']:
                        message += '\t{}\n\t\tEnabled: {}\n\t\tChannel: #{}\n'.format(event, all_settings['events'][event]['enabled'], context.guild.get_channel(all_settings['events'][event]['channel']).name if all_settings['events'][event]['channel'] else None)
                        c -= 1
            message += '```'
            guild = context.guild
